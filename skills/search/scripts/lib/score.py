"""Popularity-aware scoring for search skill."""
...
def _score_engagement_items(items, compute_engagement_fn):
    """Shared scoring logic for Reddit and X items.

    Args:
        items: List of items with engagement, relevance, date, date_confidence
        compute_engagement_fn: Function to compute raw engagement from item.engagement
...
def score_reddit_items(items: List[schema.RedditItem]) -> List[schema.RedditItem]:
    ...
    return _score_engagement_items(items, compute_reddit_engagement_raw)


def score_x_items(items: List[schema.XItem]) -> List[schema.XItem]:
    """Compute scores for X items."""
    return _score_engagement_items(items, compute_x_engagement_raw)
