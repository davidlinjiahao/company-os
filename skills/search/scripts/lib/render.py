"""Output rendering for search skill."""
...
OUTPUT_DIR = Path.home() / ".local" / "share" / "search-social" / "out"
...
def render_compact(report: schema.Report, limit: int = 15, missing_keys: str = "none") -> str:
    ...
    if report.mode == "web-only":
        ...
        lines.append("- Edit `~/.claude/.env` to add keys")
        lines.append("---")
        lines.append("")
...
def render_context_snippet(report: schema.Report) -> str:
    ...
    lines.append("")
    lines.append("*See full report for detailed sources.*")
    lines.append("")
