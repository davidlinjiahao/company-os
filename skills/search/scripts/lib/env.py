"""Environment and API key management for search skill."""
...
def get_config() -> Dict[str, Any]:
    ...
    config = {
        ...
        'FIRECRAWL_API_KEY': os.environ.get('FIRECRAWL_API_KEY') or file_env.get('FIRECRAWL_API_KEY'),
...
def validate_sources(requested: str, available: str, include_web: bool = False) -> tuple[str, Optional[str]]:
    ...
    if available == 'web':
        if requested == 'auto':
            ...
            return 'web', f"No API keys configured. Using WebSearch fallback. Add keys to ~/.claude/.env for Reddit/X."
