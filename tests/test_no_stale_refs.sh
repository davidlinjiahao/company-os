...
# Former internal org strings must never reappear in active files.
# Pattern assembled at runtime so this guard does not itself embed the literal.
_org="abun""dance"
check_no_ref "ross-${_org}-" "former path prefix"
check_no_ref "${_org}-company" "former org slug"
check_no_ref "${_org}\\.company" "former workspace domain"
