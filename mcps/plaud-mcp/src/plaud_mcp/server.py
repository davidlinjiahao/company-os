...
def _validate_file_id(file_id: str) -> str:
    if not file_id or not _FILE_ID_RE.match(file_id):
        raise PlaudAPIError(400, f"Invalid file_id format: {file_id!r}")
    return file_id
...
async def get_files(
    ...
    Note: Fetches up to `limit` most recent files, then filters by date client-side.
    """
...
async def get_file(file_id: str) -> dict[str, Any]:
    ...
    return _format_file(await client.get_file(_validate_file_id(file_id)))
...
async def get_transcript(file_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        data = await client.get_transcript(fid)
        ...
        return {"file_id": fid, "transcript": str(data)}
...
async def get_summary(file_id: str) -> dict[str, Any]:
    ...
    try:
        fid = _validate_file_id(file_id)
        data = await client.get_summary(fid)
        if isinstance(data, str):
            return {"file_id": fid, "content": data}
        return {
            "file_id": fid,
...
async def rename_file(file_id: str, new_name: str) -> dict[str, Any]:
    """Rename a Plaud file. Returns the API response."""
    try:
        fid = _validate_file_id(file_id)
        result = await client.rename_file(fid, new_name)
        return {"file_id": fid, "new_name": new_name, "result": result}
    except PlaudAPIError as e:
        return {"file_id": file_id, "error": str(e)}
    ...
    async def _search_file(file: dict[str, Any]) -> dict[str, Any] | None:
        async with sem:
            try:
                ...
                logger.warning("Failed to search file %s: %s", file.get('id'), e)
            return None
