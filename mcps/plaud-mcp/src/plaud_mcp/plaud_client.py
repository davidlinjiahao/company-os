...
def _find_plaud_pid() -> int | None:
    try:
        ...
        logger.debug("Failed to find Plaud PID: %s", e)
    return None
...
def _enable_inspector(pid: int) -> bool:
    try:
        ...
        logger.debug("Failed to enable inspector: %s", e)
        return False
...
def _get_ws_url() -> str | None:
    try:
        ...
        logger.debug("Inspector not available: %s", e)
    return None
...
class PlaudClient:
    ...
    async def _cdp_eval(self, js_expression: str, _retry: bool = True) -> Any:
        ...
        try:
            ...
            self._ws_url = None
            if _retry:
                logger.debug("CDP connection failed, retrying: %s", e)
        ...
        js = f"""
            (async () => {{
                try {{
                    ...
                    const opts = {{}};
                    if ({params_js} !== undefined) opts.params = {params_js};
                    if ({method_js} !== 'GET') opts.method = {method_js};
                    if ({body_js} !== undefined) opts.body = {body_js};
                    const result = await fetchFn({endpoint_js}, opts);
    ...
    async def _fetch_content_url(self, url: str) -> Any:
        """Fetch content from a signed S3 URL, handling gzip and non-JSON content."""
        async with httpx.AsyncClient() as client:
            ...
            try:
                ...
                # Summaries are markdown, not JSON
    ...
    async def _get_content_by_type(self, file_id: str, data_type: str, label: str) -> Any:
        ...
        detail = await self.get_file(file_id)
    ...
    async def get_files(
        ...
        skip: int = 0,
        limit: int = 100,
        is_trash: int = 2,  # 0=not trashed, 1=trashed, 2=all
    ...
    async def rename_file(self, file_id: str, new_name: str) -> dict[str, Any]:
        """Rename a Plaud file via PATCH /file/{file_id}."""
        return await self._fetch(f"/file/{file_id}", method="PATCH", body={"filename": new_name})
